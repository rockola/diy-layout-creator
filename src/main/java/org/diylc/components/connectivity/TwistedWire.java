...
public class TwistedWire extends AbstractCurvedComponent<Void> implements IContinuity {
  ...
  public void drawIcon(Graphics2D g2d, int width, int height) {
    ...
    CubicCurve2D curve1 =
        new CubicCurve2D.Double(2, height - 2, width - 4, height - 4, 4, 4, width - 2, 2);
    CubicCurve2D curve2 =
        new CubicCurve2D.Double(2, height - 2, 4, 4, width - 4, height - 4, width - 2, 2);
  ...
  public void recalculate(
      Path2D path, float thickness, Area firstLeadArea, Area secondLeadArea, boolean stripe) {
    ...
    while (!iterator.isDone()) {
      ...
      switch (type) {
        ...
        case PathIterator.SEG_LINETO:
          curves.add(
              new CubicCurve2D.Double(
                  current.x,
                  current.y,
                  (current.x + coords[0]) / 2,
                  (current.y + coords[1]) / 2,
                  (current.x + coords[0]) / 2,
                  (current.y + coords[1]) / 2,
                  coords[0],
                  coords[1]));
        ...
        case PathIterator.SEG_QUADTO:
          curves.add(
              new CubicCurve2D.Double(
                  current.x, current.y, coords[0], coords[1], coords[0], coords[1], coords[2],
                  coords[3]));
        ...
        case PathIterator.SEG_CUBICTO:
          curves.add(
              new CubicCurve2D.Double(
                  current.x, current.y, coords[0], coords[1], coords[2], coords[3], coords[4],
                  coords[5]));
    ...
    for (int i = 0; i < polygon.size(); i++) {
      ...
      current1.quadTo(
          ...
          centerY + offset * Math.sin(theta1),
          line.getX2(),
          line.getY2());
      current2.quadTo(
          centerX + offset * Math.cos(theta2),
          centerY + offset * Math.sin(theta2),
          line.getX2(),
          line.getY2());
