...
public class DrawingManager {
    ...
    private Theme theme = (Theme) DIYLC.getObject(IPlugInPort.THEME_KEY,
     ...
     * @return
     */
    public List<IDIYComponent<?>> drawProject(Graphics2D g2d,
					      Project project,
	...
	Dimension d = getCanvasDimensions(project,
					  zoom,
	...
	if (drawOptions.contains(DrawOption.EXTRA_SPACE)) {
	    ...
	    g2d.setStroke(ObjectCache.getInstance().fetchStroke(borderThickness,
								new float[] {
								    borderThickness * 4,
								    borderThickness * 4,
								},
	    ...
	    extraSpaceRect = new Rectangle2D.Double(extraSpace,
						    extraSpace,
						    dInner.getWidth(),
						    dInner.getHeight());
	...
	for (IDIYComponent<?> component : project.getComponents()) {
		...
		try {
		    component.draw(g2dWrapper,
				   state,
				   drawOptions.contains(DrawOption.OUTLINE_MODE),
				   project,
				   g2dWrapper);
		    if (g2dWrapper.isTrackingContinuityArea()) {
			LOG.info("Component {} of type {} did not stop tracking continuity area.",
				 component.getName(),
	...
	for (IDIYComponent<?> c : project.getComponents()) {
		...
		for (int i = 0; i < c.getControlPointCount() - 1; i++)
			...
			if (((IContinuity)c).arePointsConnected(i, j))
			    connections.add(new Connection(c.getControlPoint(i),
							   c.getControlPoint(j)));
	    }
