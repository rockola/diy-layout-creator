...
public class CloudPresenter {
	...
	try {
	    String res = getService().uploadProject(currentUsername(),
						    currentToken(),
						    getMachineId(),
						    project.getName(),
	...
	try {
	    String res = getService().deleteProject(currentUsername(),
						    currentToken(),
						    getMachineId(),
						    projectId);
	    if (!res.equals(SUCCESS))
	...
	Object res;
	try {
	    res = getService().getUserDetails(currentUsername(),
					      currentToken(),
					      getMachineId());
	} catch (Exception e) {
	    throw new CloudException(e);
	...
	String res;
	try {
	    res = getService().updateUserDetails(currentUsername(),
						 currentToken(),
						 getMachineId(),
						 email, website, bio);
	} catch (Exception e) {
    ...
    public List<ProjectEntity> search(String criteria, String category, String sortOrder,
				      int pageNumber, int itemsPerPage)
	throws CloudException {
	LOG.info(String.format("search(%1$s,%2$s,%3$s,%4$d,%5$d)",
			       criteria, category, sortOrder,
			       pageNumber, itemsPerPage));
	try {
	    Object res = getService().search(criteria, category, pageNumber,
					     itemsPerPage, sortOrder, null, null);
