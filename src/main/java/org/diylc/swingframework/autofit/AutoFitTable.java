...
public class AutoFitTable extends JTable {
  ...
  private static final Logger LOG = LogManager.getLogger(AutoFitTable.class);
      ...
      for (int j = 0; j < getRowCount(); j++) {
        ...
        if (renderer != null) {
          Component rendererComponent =
              renderer.getTableCellRendererComponent(this, value, false, false, rowIdx, columnIdx);
          try {
            Method getText = rendererComponent.getClass().getMethod("getText");
            String text = (String) getText.invoke(rendererComponent);
            int cellWidth =
                getGraphics().getFontMetrics(rendererComponent.getFont()).stringWidth(text) + 12;
            if (cellWidth > width) {
              width = cellWidth;
            }
          } catch (Exception e) {
