...
public abstract class ReflectiveClone {
  ...
  public static Object clone(final Object obj) {
    return clone(obj, new IdentityHashMap(), new HashMap());
  }
  ...
  /**
   * The workhorse behind clone(Object). This method is mutually recursive with {@link
   * #setFields(Object, Object, Field[], boolean, Map, Map)}.
   *
    ...
    { // clone = objClass.newInstance () can't handle private constructors
      ...
      if (noarg == null) {
        try {
          ...
          throw new RuntimeException(
              "class [" + objClass.getName() + "] has no noarg constructor: " + e.toString());
        }
      }
      ...
      if (!metadata.m_noargConstructorAccessible
        ...
        try {
          ...
          throw new RuntimeException(
              "cannot access noarg constructor ["
                  + noarg
                  + "] of class ["
                  + objClass.getName()
                  + "]: "
   ...
   * @param accessible 'true' if all 'fields' have been made accessible during this traversal
   */
    ...
    for (int f = 0, fieldsLength = fields.length; f < fieldsLength; ++f) {
      ...
      try // to clone and set the field value:
      {
        ...
        if (value == null) {
          ...
          if (DEBUG)
            System.out.println(
                "set field #" + f + ", [" + field + "] of object [" + dest + "]: NULL");
        } else {
          ...
          if (DEBUG)
            System.out.println(
                "set field #" + f + ", [" + field + "] of object [" + dest + "]: " + value);
        }
      } catch (Exception e) {
