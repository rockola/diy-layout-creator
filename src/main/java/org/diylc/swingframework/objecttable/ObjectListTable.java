...
/**
 * Customized {@link JTable} that shows objects displayed as rows in the table. Cells in each rows
...
public class ObjectListTable<T> extends AutoFitTable {
   ...
   * </ul>
   *
   * The example below shows a simple class that has two fields. We are making a table with three
   ...
   * </code> <br>
   * <br>
   * The resulting table will contain columns named "X", "Y" and "Delete" in that order. Column
   * "Delete" contains buttons that trigger the action. Column "X" is editable because it has a
   * setter defined. Event will be fired back the user though {@link IActionProcessor}.<br>
   * <br>
   *
  ...
  public ObjectListTable(
    ...
    for (Integer idx : ((ObjectListTableModel<T>) getModel()).getActionColumns()) {
      getColumnModel()
          .getColumn(idx)
          .setCellRenderer(
              new ActionTableCellRenderer(
                  this.clickListener.getActionIcon(getModel().getColumnName(idx)),
                  this.clickListener.getActionLabel(getModel().getColumnName(idx))));
    }
  ...
  /**
   * Sets the data to the table. A new row is created for each object in the list.
   *
   * @param data
   */
  @SuppressWarnings("unchecked")
