...
public final class Utilities {
	...
	if (Component.class.isAssignableFrom(clazz)) {
		...
		if (pds[i].getName().equals("cursor")) {
			...
			Method setter = Component.class.getDeclaredMethod("setCursor", new Class[] {Cursor.class});
	...
	String c = (String) initNameAndValues().keyToString.get(Integer.valueOf(stroke.getKeyCode()));
	if (c == null) {
	...
	if (((isMac()) && ((modifiers & 0x4) != 0)) || ((!isMac()) && ((modifiers & 0x2) != 0))) {
	    buf.append('D');
	    b = true;
	}
	if (((isMac()) && ((modifiers & 0x2) != 0)) || ((!isMac()) && ((modifiers & 0x8) != 0))) {
	    buf.append('O');
	    b = true;
	}
	...
	return new Rectangle(bounds.x + (bounds.width - componentSize.width) / 2, bounds.y
	...
	if (fileToPath != null) {
		...
		pathsGet = Class.forName("java.nio.file.Paths").getMethod("get", new Class[] {URI.class});
	...
	if (fileToPath != null) {
	    try {
		URI u = (URI) pathToUri.invoke(fileToPath.invoke(f, new Object[0]), new Object[0]);
		if (u.toString().startsWith("file:///")) {
		}
		return new URI(u.getScheme(), u.getUserInfo(), u.getHost(),
			       u.getPort(), u.getPath(), u.getQuery(),
			       u.getFragment());
	    } catch (Exception x) {
		LOG.debug("could not convert " + f + " to URI:" + x);
	    }
	}
	...
	if (pathsGet != null) {
	    try {
		return (File) pathToFile.invoke(pathsGet.invoke(null, new Object[] {u}),
						new Object[0]);
	    } catch (Exception x) {
		LOG.debug("could not convert " + u + " to File:" + x);
	    }
	}
