...
/**
 * Undecorated {@link JDialog} that hooks onto a {@link JScrollPane} and renders a thumbnail of the
 * Viewport. When mouse is moved inside the dialog it shows visible rect around the cursor. When
 * clicked Viewport position is moved to the mouse position.
 *
...
public class NavigateDialog extends JDialog {
  ...
  /**
   * {@link JComponent} that renders thumbnail of the viewport component and visible rectangle.
   *
   * @author Branislav Stojkovic
   */
  class DrawComponent extends JComponent {
      ...
      addMouseListener(
          new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
              int x =
                  (int)
                      (getMousePosition().x / scaleRatio
                          - DrawComponent.this.scrollPane.getVisibleRect().width / 2);
              if (x
                  > dim.width
              ...
              int y =
                  (int)
                      (getMousePosition().y / scaleRatio
                          - DrawComponent.this.scrollPane.getVisibleRect().height / 2);
              if (y
                  > dim.height
      ...
      do {
        ...
        try {
          ...
          Rectangle visibleRect =
              ((JComponent) scrollPane.getViewport().getView()).getVisibleRect();
    ...
    public void createBufferImage() {
      bufferImage =
          screenGraphicsConfiguration.createCompatibleVolatileImage(getWidth(), getHeight());
    }
  }
}
